                    ...
                    <thead>
                        <tr>
                            <td align="center" class="text-center" style="border:none;text-align: center;padding-bottom: 10px;">
                                <img src="<?php if($setting->logo == '') {
                                    echo base_url('assets/img/icons/mini-logo.png');
                                } else { echo base_url('/').$setting->logo; 
                                } ?>" width="100px;">
                            </td>
                        </tr>
                        <tr>
            ...
            <table width="99%" class="payrollDatatableReport table table-striped table-bordered table-hover">
                ...
                <tbody class="employee_salary_chart">
                  ...
                  <tr>
                    ...
                    <td class="text-left"><?php echo $currency.' '.number_format($row->basic_salary_pro_rated, 0, ',', '.');?></td>
                    <td class="text-left"><?php echo $currency.' '.number_format($total_benefits, 0, ',', '.');?></td>
                    <td class="text-left"><?php echo $currency.' '.number_format($row->transport_allowance_pro_rated, 0, ',', '.');?></td>
                    <td class="text-left"><?php echo $currency.' '.number_format($row->gross_salary, 0, ',', '.'); ?></td>
                    <td class="text-left"><?php echo $currency.' '.number_format($row->income_tax, 0, ',', '.'); ?></td>
                    <td class="text-left"><?php echo $currency.' '.number_format($row->soc_sec_npf_tax, 0, ',', '.'); ?></td>
                    <td class="text-left"><?php echo $currency.' '.floatval($row->employer_contribution);?></td>
                    <td class="text-left"><?php echo $currency.' '.number_format($row->loan_deduct, 0, ',', '.');?></td>
                    <td class="text-left"><?php echo $currency.' '.number_format($row->salary_advance, 0, ',', '.');?></td>
                    
                    <td class="text-left"><?php echo $currency.' '.number_format($total_deductions, 0, ',', '.');?></td>

                    <td class="text-left"><?php echo $currency.' '.number_format($row->net_salary, 0, ',', '.');?></td>
                  </tr>
