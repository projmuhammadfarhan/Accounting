...
public function bdtask_product_details($product_id = null){
        ...
        $this->db->query("SELECT a.*,b.*,COUNT(b.quantity) as hasil,sum(b.total_price) as total_price, c.customer_name FROM invoice a
LEFT JOIN invoice_details b on b.invoice_id = a.invoice_id
LEFT JOIN customer_information c on c.customer_id = a.customer_id");
        $query = $this->db->get();
        ...
        if (!empty($salesData)) {
            foreach ($salesData as $k => $v) {
                $salesData[$k]['final_date'] = $query[$k]['date'];
                $totalSales = ($totalSales + $query[$k]['hasil']);
                $totaSalesAmt = ($totaSalesAmt + $query[$k]['total_price']);
            }
        }
       ...
       $data = array(
            ...
            'price'               => number_format($details_info[0]['price'] , 0, ',', '.'),
